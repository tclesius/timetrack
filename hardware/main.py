...
def loading_bar(xmax, seconds):
    step_duration = seconds / xmax
    x = 0
    for _ in range(xmax):
        x += 1
        oled.hline(0, 30, int(x), 2)  # Draw the loading bar
        oled.show()
        sleep(step_duration)
...
while True:
    ...
    if stat == reader.OK:
        ...
        if stat == reader.OK:
            ...
            sleep(0.1)
            buzzer.duty_u16(0)
            if not bool(json_data.get('paired')):
                print(json_data.get('paired'))
                oled.fill(0)
                oled.text(f"Pairing token:",10,0)
                oled.text(f"{json_data.get('token')}",30,15)
                oled.show()
                loading_bar(128, 30)
                oled.fill(0)
                oled.text("Bring TAG closer...",0,0)
